...
#' @return An S3 object of class `"secureguard"`.
#' @export
...
#'   redacted text).
#' @return A list of class `"guardrail_result"`.
#' @export
...
compose_guardrails <- function(..., mode = c("all", "any")) {
  ...
  for (i in seq_along(guardrails)) {
    if (!inherits(guardrails[[i]], "secureguard")) {
...
run_guardrail <- function(guardrail, x) {
  if (!inherits(guardrail, "secureguard")) {
...
#' @export
print.guardrail_result <- function(x, ...) {
