...
This vignette covers advanced usage patterns for secureguard: building custom
guardrails, composing them into layered defenses, assembling full pipelines,
and integrating with securer for sandboxed execution.

If you are new to secureguard, start with `vignette("secureguard")` for an
overview of the three defense layers.

## Creating Custom Guardrails

Every guardrail in secureguard is an S7 object of class `secureguard` with
four properties: `name`, `type`, `check_fn`, and `description`. The
...
Suppose your agent generates SQL queries. You want a guardrail that blocks
common SQL injection patterns before any query reaches a database.
...
single composite guardrail. This is useful when you want to treat a group of
checks as one unit.
...
When `mode = "any"`, the composite passes if **any** child guardrail passes.
This is useful for allowlist-style checks where multiple patterns are
acceptable:
...
A pipeline bundles guardrails for all three layers -- input, code, and output
-- into a single object with dedicated methods for each stage.
...
Here is how a pipeline fits into a typical agent turn. This pattern processes
one user request through all three layers sequentially, short-circuiting on
failure:
...
Custom guardrails compose seamlessly with built-in ones. You can mix them in
`compose_guardrails()`, `check_all()`, and `secure_pipeline()`:
...
[securer](https://github.com/ian-flores/securer) package to guard sandboxed R
execution sessions. securer is a suggested dependency -- all of the patterns
above work without it. The integration layer adds two capabilities:
pre-execution hooks and output guarding after execution.
...
Use different sensitivity levels for different contexts. For example, a public
chatbot may need stricter injection detection than an internal tool:
...
Output guardrails support three actions: `"block"`, `"redact"`, and `"warn"`.
Combining these in a pipeline lets you redact recoverable issues while blocking
critical ones:
...
| Same-type composition | `compose_guardrails()` | Merge guards into one |
| Batch check | `check_all()` | Individual results per guard |
| Full pipeline | `secure_pipeline()` | Three-layer defense |
